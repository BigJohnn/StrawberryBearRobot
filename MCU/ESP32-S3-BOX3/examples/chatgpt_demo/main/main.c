...
esp_err_t start_openai(uint8_t *audio, int audio_len)
{
    ...
    // if command detected, then send to MPU
    // todo: use NLP to enhance result
    ESP_LOGI(TAG, "prompt text===%s", text);
    if(strstr(text, "动")) {
        ESP_LOGI(TAG, "[command prompt detected] move");
        set_text_cmd("move");
        goto err;
    }
    else if(strstr(text, "停")) {
        ESP_LOGI(TAG, "[command prompt detected] stop");
        set_text_cmd("stop");
        goto err;
    }
